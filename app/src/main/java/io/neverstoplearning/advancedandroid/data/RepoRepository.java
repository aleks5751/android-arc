...
public class RepoRepository {
    ...
    private final Map<String, ContributorDetails> cachedContributorDetails = new HashMap<>();
    ...
    public Single<ContributorDetails> getContributorDetails(String userName) {
        return Maybe.concat(cachedContributorDetails(userName), apiContributorDetails(userName))
                .firstOrError()
                .subscribeOn(scheduler);
    }
    ...
    private Maybe<ContributorDetails> apiContributorDetails(String userName) {
        return repoRequesterProvider.get().getContributorDetails(userName)
                .doOnSuccess(contributorDetails -> cachedContributorDetails.put(userName, contributorDetails))
                .toMaybe();
    }

    private Maybe<ContributorDetails> cachedContributorDetails(String userName) {
        return Maybe.create(e -> {
            if (cachedContributorDetails.containsKey(userName)) {
                e.onSuccess(cachedContributorDetails.get(userName));
            }
            e.onComplete();
        });
    }
